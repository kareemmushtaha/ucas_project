...
@section('TRY')
    ...
    <div class="col-12">
        <div class="card">
            <button class="btn btn-primary" type="submit"><a href="ImgAllRestaurant/create" style="color:wheat;">
                ...
                <table class="table table-hover table table-striped table-bordered example">
                    ...
                    @foreach($data as $row)
                        <tr>
                            <td>{{ $row->id }}</td>
                            <td>{{ $row->details }} </td>
                            <td> {{$row->getRestaurantByImg->name}}</td>
                            @if($row->img!="NULL")
                            ...
                            <td>
                                <button class="btn btn-primary" type="submit"><a
                                        href="ImgAllRestaurant/{{ $row->id }}/edit/"
                                        class=" btn-primary"><i class="fa fa-edit"> Update</i></a>
                                </button>
                            </td>
                            <td>
                                <form action="{{action('Admin\ImgRestaurantController@destroy', $row->id)}}"
                                      method="post">
                                    {{csrf_field()}}
                                    <input name="_method" type="hidden" value="DELETE">
                                    <button class="btn btn-danger"
                                            onclick="return confirm(' ؟  هل أنت متأكد من عملية حذف الصورة التي تتبع للمطعم {{ $row->getRestaurantByImg->name }}')"
